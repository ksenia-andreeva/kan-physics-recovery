...
### 3. Попытка обобщения без инженерии признаков (MultKAN с 5 исходными переменными)
- **Задача:** проверить, способен ли MultKAN самостоятельно открыть мультипликативную структуру закона, располагая только исходными переменными (x, v, k, c, 1/m) без производных признаков.
- **Метод:** использован MultKAN `[5, [4,2], 1]`, grid=5, k=3 (два мультипликативных нейрона). Обучение: 800 шагов Adam + 500 шагов L‑BFGS с лёгкой энтропийной регуляризацией, затем многоэтапная обрезка и дообучение.
- **Результат:** MSE на тесте ~2.1, предсказания хаотичны, символьная формула не интерпретируема. Сеть не смогла самостоятельно сконструировать нужные произведения и деление, что подчёркивает необходимость инженерии признаков даже при наличии узлов умножения.

### 4. Использование kanpiler для встраивания известной формулы
- **Задача:** проверить, даёт ли компиляция точного закона a = -(k/m)x - (c/m)v в архитектуру KAN компактную линейную формулу после символьной регрессии.
- **Метод:** с помощью `kanpiler` формула преобразована в KAN‑сеть с пятью входами (x, v, k, c, m). Сеть обучена Adam (700 шагов) на 6000 точках, затем выполнена символьная регрессия с библиотекой `['x']`.
- **Результат:** MSE 5.6*10^{-3}, однако символьная регрессия выдала громоздкое нелинейное выражение с дробями и экспонентами. Даже точное знание закона не гарантирует компактную формулу без дополнительной постобработки.

### 5. Обобщённая модель с 11 признаками (`kan_multiparam_11features.ipynb`)
